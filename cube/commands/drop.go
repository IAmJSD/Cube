...
func init() {
	commandprocessor.Commands["drop"] = &commandprocessor.Command{
		...
		Function: func(Args *commandprocessor.CommandArgs) {
			...
			if err == nil {
				// Drop the currency.
				drops.ExecuteDrop(Args.Channel.ID, Args.Channel.GuildID, Args.Session, i, Args.Prefix, Args.Message.Author.Mention()+" has dropped "+*cur.Emoji+" "+strconv.Itoa(i)+".", cur.DropsImage)
			}
		},
	}
}
