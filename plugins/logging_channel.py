...
def Plugin(app):
    ...
    @app.command("Allows you to set a logging channel.", requires_management=True, usage="[channel]")
    async def logging_channel(app):
        if app.args == []:
            ...
            if channel is None:
                ...
                try:
                    test_msg = await channel.send("This is a test of the logging channel.")
                    ...
                    await app.say(embed=app.create_embed("Could not send a test message.",
                        "Make sure the bot has read and write permssions to the channel.",
                                                                            error=True))
                    return
                await app.run_mysql("DELETE FROM logging_channels WHERE server_id = %s", (app.message.guild.id, ), commit=True)
                await app.run_mysql("INSERT INTO logging_channels (server_id, channel_id) VALUES (%s, %s)",
                                                        (app.message.guild.id, channel.id, ), commit=True)
                embed = app.create_embed("Logging channel set:",
                f"{app.message.author.mention} set the logging channel to {channel.mention}.",
                success=True)
                await app.say(embed=embed)
                await app.attempt_log(app.message.guild.id, embed=embed)
    # Allows you to set a logging channel.
